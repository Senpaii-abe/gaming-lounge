...
def admin_login(request):
    if request.method == 'POST':
        ...
        if user is not None and user.is_staff:
            ...
            messages.error(request, 'Invalid email or password. Please try again.')  # Add an error message
            return render(request, 'admin/login.html', {'email': email})  # Pass email to the template context for prefilling the form
...
def admin_users(request):
    ...
    new_users_november = User.objects.filter(date_joined__gte=start_date, date_joined__lt=end_date).count()
...
def edit_user(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('admin_users')  # Redirect to user list page or wherever needed
    else:
